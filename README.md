...
a. Confirm that the output Python file is successfully generated. Rename the output file according to your need.
   - If the name of the output file is not changed, the autogenerated script after running the protocol generator will be named 'RNA_interference.py' uniformly. This may cause problem when you run the protocol generator a 2nd time for a different process/parts as it overwrites the original output script.
...
### Non-Removable Sheets
   ...
   - NOTICE: If the blue cell in a row under `Non-Removable Sheets` is filled, all other cells in the same row (under the green column headings) must not be left empty. Replace any empty cells with `/`. Don't replace with `None`.
...
1. **Labwares**  
   ...
   - Examples:
      ...
      **!!!`Partial Tip Pickup` commands the multi-channel pipettes to switch the nozzle configuration to single layout. This command may cause pipettes to collide with labwares at certain positions. This limits the labware placement for this protocol.!!!**

   **Layout for Part I:**

   <img width="672" alt="{05C12578-6780-4AD6-B13E-CA8F2445A103}" src="https://github.com/user-attachments/assets/bd7c4ff4-5395-4fdc-b726-ef5b21348a47">

   **Layout for Part II and Part III:**

   <img width="672" alt="{4CD6845C-D5CC-41EF-87B4-C5A7A674CC10}" src="https://github.com/user-attachments/assets/4061dd63-7f58-4992-9133-deaaf4ce72af">
...
2. **Problem**: The `Partial Tip Pickup` function causes collision between pipettes and labwares at certain positions.
   - **Solution**: Limit the labware placements for user inputs.
     
3. . **Problem**: Inputs in the `names` column of the Excel file may contain empty spaces or special characters, which cannot be directly used in Python scripts.  
...
4. **Problem**: The experimental steps specified in the article sometimes require swapping the order of siRNA and transfection lipid mix additions for different purposes.  
   - **Solution**: Although strict reagent assignments for `Step X` sheets are reserved. The actual experimental order is defined in the `Addition_V&T` sheet as `orders`.

5. **Problem**: During wet run simulations, liquids would drop out during transfer.  
...
6. **Problem**: Liquid droplets sometimes remain on the outside of pipette tips.  
   - **Solution**: Implemented the `touch_tip` function to shake off residual liquid from the outside of the tips.

7. **Problem**: Small volumes (<100 µL) are challenging to mix effectively with pipette tips.  
...
- Edit the codes so that `Step X` correlates with `X` specified in `orders` of `Addition_V&T` sheet.

Limitation on Opentron Platform
- Develop code to enable the Opentrons robot to automatically pause and notify the user when it requires intervention, such as replacing experimental plates on the platform. This allows the same procedure to be repeated on new plates without restarting the program. Alternatively, extend the Opentrons deck to accommodate additional labware for increased throughput.
